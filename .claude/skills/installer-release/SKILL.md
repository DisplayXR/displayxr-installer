...
subagent runs from the user's `displayxr-installer` checkout.

### Subagent prompt template
```
Run the installer-release skill at .claude/skills/installer-release/SKILL.md
(or your user-level ~/.claude/skills/installer-release/SKILL.md).
Bundle tag: [TAG]
Prerelease flag: [true|false]
Run from /Users/david.fattal/Documents/GitHub/displayxr-installer
(or wherever the local installer checkout lives).
...
### Step 1.1: Confirm we're in the installer repo
```bash
REMOTE=$(git remote get-url origin)
[[ "$REMOTE" == *displayxr-installer* ]] || { echo "Run from displayxr-installer"; exit 1; }
```
...
case "$ARG" in
  ...
  patch|minor|major)      LATEST=$(git tag --sort=-creatordate | grep -E '^v[0-9]+\.[0-9]+\.[0-9]+$' | head -1)
                          TAG=$(bump "$LATEST" "$ARG") ;;
  *)                      echo "Bad version spec"; exit 1 ;;
esac
```

### Step 1.3: Confirm versions.json is in sync with runtime
...
### Step 1.4: Show the bundle composition the user is about to ship
```bash
